import { makeFacts } from "./logic_lib.ts";
import fs from "fs/promises";

const MORE_URI = "https://public.opendatasoft.com/api/explore/v2.1/catalog/datasets/geonames-all-cities-with-a-population-1000/exports/json?lang=en&timezone=America%2FChicago";
...
export async function acquireData() {
    if (await fs.access(LOCAL_DATA).then(() => true).catch(() => false)) return;
    console.log("downloading data...");
    const start = Date.now();
    await fetch(MORE_URI).then(res => res.text()).then(text => fs.writeFile(LOCAL_DATA, text));
...
export async function loadData() {
    ...
    const start = Date.now();
    const rawData = await fs.readFile(LOCAL_DATA).then(buffer => buffer.toString());
    const data = JSON.parse(rawData);
    const countryCodeMap = {} as Record<string, string>;
    for (const row of data) {
        const id = row.geoname_id;

        country_state_city.set(id, row.country_code, row.admin1_code, row.ascii_name);
