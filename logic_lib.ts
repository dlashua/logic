...
// Stream type for results (now an async generator)
export type Stream = AsyncGenerator<Subst>;

// Goal type (returns an async generator)
...
// Run: get results for a goal as an async generator, with ergonomic object mapping using logic variable IDs
...
// Helper: remove duplicates from an async generator based on a key function
...
export const not = (g: Goal): Goal => async function* (s: Subst) {
    const it = g(s);
    if ((await it.next()).done) yield s;
};
...
export function ife(goal_if: Goal, goal_else: Goal): Goal {
    return async function* (s: Subst) {
        let any = false;
        for await (const s1 of goal_if(s)) {
            any = true;
            yield s1;
        }
        if (!any) {
            yield* await goal_else(s);
        }
    };
}
