import type { Term, Subst, Goal, Observable } from"logic";
import { isVar, walk, GOAL_GROUP_CONJ_GOALS, GOAL_GROUP_ALL_GOALS } from "logic"
import { Logger, getDefaultLogger } from "logic";
import { SimpleObservable } from "logic";
import type { 
  DataStore, 
  GoalRecord, 
  GoalManager, 
...
export class AbstractRelation<TOptions extends RelationOptions = RelationOptions> {
      ...
      return new SimpleObservable<Subst>((observer) => {
        ...
        const batchProcessor = this.createBatchProcessor({
          ...
          onFlush: async (batch) => {
            ...
            // Process batch
            const rows = await this.executeQueryForSubstitutions(goalId, queryObj, batch as Subst[]);
            ...
            if (myGoal && representativeSubst) {
              const relatedGoals = await this.findRelatedGoals(myGoal, representativeSubst as Subst);
              cacheCompatibleGoals = this.findCacheCompatibleGoals(myGoal, relatedGoals, representativeSubst as Subst);
            }
            
            await this.processFreshRows(goalId, queryObj, rows, batch as Subst[], observer, cacheCompatibleGoals);
  ...
  private async executeQueryForSubstitutions(
    ...
    return await this.buildAndExecuteQuery(goalId, queryObj, substitutions, mergeCompatibleGoals, cacheCompatibleGoals);
  }
  ...
  private async buildAndExecuteQuery(
    ...
    const whereClauses = await this.collectWhereClausesFromSubstitutions(queryObj, substitutions);
    ...
    if (mergeCompatibleGoals.length > 0) {
      ...
      if (myGoal) {
        ...
        const goalWhereClauses = await this.collectAllWhereClauses(allGoalsToMerge, substitutions[0]);
    ...
    const columns = this.collectColumnsFromGoals(queryObj, cacheCompatibleGoals, mergeCompatibleGoals);
   ...
   * Find related goals for merging and caching
   */
  private async findRelatedGoals(myGoal: GoalRecord, s: Subst): Promise<{ goal: GoalRecord, matchingIds: string[] }[]> {
  ...
  private collectWhereClausesFromSubstitutions = collectWhereClausesFromSubstitutions;
