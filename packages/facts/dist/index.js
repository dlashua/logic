...
var MemoryRelation = class {
  ...
  createGoal(query) {
    return (input$) => {
      ...
      return input$.pipe(
        (0, import_operators.mergeMap)((s) => {
          const results = this.queryFacts(query, s);
          return results.length > 0 ? (0, import_rxjs.from)(results) : import_rxjs.EMPTY;
        })
      );
    };
  }
  queryFacts(query, substitution) {
    const results = [];
  ...
  getCandidateIndexes(query) {
    ...
    let candidateIndexes = /* @__PURE__ */ new Set();
    if (indexedPositions.length > 0) {
      ...
      for (const pos of indexedPositions) {
        ...
        if (candidateIndexes.size === 0 && indexedPositions.indexOf(pos) === 0) {
          candidateIndexes = new Set(factNums);
        } else {
          candidateIndexes = indexUtils2.intersect(candidateIndexes, factNums);
          if (candidateIndexes.size === 0) break;
        }
      }
    }
...
import { eq } from "@codespiral/logic";
