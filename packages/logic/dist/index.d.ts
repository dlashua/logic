...
declare class SimpleObservable<T> implements Observable<T> {
    ...
    static fromAsync<T>(asyncProducer: (observer: Observer<T>) => Promise<(() => void) | undefined>): Observable<T>;
    ...
    reduce<V>(reducer: (accumulator: V, value: T) => V, initialValue: V): SimpleObservable<V>;
    share(bufferSize?: number): SimpleObservable<T>;
    merge(other: SimpleObservable<T>): SimpleObservable<T>;
    toArray(): Promise<T[]>;
    firstFrom(): Promise<T>;
    pipe<V>(next_observable: (input$: SimpleObservable<T>) => SimpleObservable<V>): SimpleObservable<V>;
    lastFrom(): Promise<T>;
}
type ObsToObs = (input: SimpleObservable<any>) => SimpleObservable<any>;
