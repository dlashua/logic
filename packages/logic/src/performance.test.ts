...
import { and, eq, or } from "./core/combinators.js";
import {
	arrayToLogicList,
	logicList,
	lvar,
...
describe("Performance Tests", () => {
	...
	it("should benchmark basic operations", async () => {
		...
		const varResult = await benchmark(
			"Variable creation",
			() => {
				return lvar("test");
			},
		...
		const unifyResult = await benchmark(
			"Simple unification",
			async () => {
				const x = lvar("x");
				const s = new Map();
		...
		// Test walking
		const walkResult = await benchmark(
			"Walking variables",
			async () => {
				const x = lvar("x");
				const y = lvar("y");
		...
		const deepWalkResult = await benchmark(
		...
		// Test goal execution
		const goalResult = await benchmark(
			"Goal execution (eq)",
			async () => {
				const x = lvar("x");
				return await query()
		...
		const complexGoalResult = await benchmark(
			"Complex goal (and)",
			async () => {
				const x = lvar("x");
				const y = lvar("y");
				...
				return await query()
					...
					.where(($) => and(eq(x, 1), eq(y, 2), eq(z, 3)))
					.toArray();
			},
			1000,
		);
		...
		const queryResult = await benchmark(
			...
			async () => {
				return await query()
					...
					.where(($) => [or(eq($.x, 1), eq($.x, 2), eq($.x, 3)), eq($.y, 42)])
					.toArray();
			},
			500,
		);
