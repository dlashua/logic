...
import { eq, and } from '../core/combinators.js'
import { query } from '../core/query.js'
import { Subst, Term } from '../core/types.js'
import { membero } from './lists.js'
import { collect_and_process_base, group_by_streamo_base } from './aggregates-base.js'
...
describe('Aggregates Base Functions', () => {
        ...
        collect_and_process_base(
          (buffer, observer) => {
            if (buffer.length > 0) {
              ...
              const xValues = buffer.map(subst => walk(xVar, subst)).filter(v => v !== undefined);
  ...
  describe('group_by_streamo_base', () => {
    it('should group by key and aggregate values (preserve mode)', async () => {
      ...
      const groupCountGoal = (categoryTerm: Term, valueTerm: Term, countTerm: Term) =>
    ...
    it('should collect values into lists per group', async () => {
      ...
      const groupCollectGoal = (categoryTerm: Term, itemTerm: Term, itemsTerm: Term) =>
        group_by_streamo_base(
          categoryTerm, // keyVar
          itemTerm, // valueVar
          itemsTerm, // outVar
