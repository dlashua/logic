...
import {} from "../relations/lists.js";
import {
	collect_distincto,
	collecto,
	counto,
...
describe("Aggregation Relations", () => {
	...
	describe("collecto", () => {
		it("should collect all values into a logic list", async () => {
			...
			const goal = collecto(x, or(eq(x, 1), eq(x, 2), eq(x, 3)), result);
	...
	describe("counto", () => {
		it("should count the number of solutions", async () => {
			...
			const goal = counto(x, or(eq(x, 1), eq(x, 2), eq(x, 3)), count);
			const results = await query()
				.select(($) => ({
					count,
				}))
