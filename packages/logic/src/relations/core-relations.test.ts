...
import { collecto } from "./aggregates-subqueries.js";
import { not } from "./control.js";
...
describe("Core Relations", () => {
	...
	describe("membero", () => {
		...
		it("should generate array elements when array is variable", async () => {
			...
			const goal = and(eq(list, [1, 2, 3]), membero(2, list));
			const results = await query()
				.select(($) => ({
					list,
				}))
	...
	describe("collecto", () => {
		it("should collect all solutions", async () => {
			...
			const goal = collecto(x, membero(x, [1, 2, 3]), result);
			const results = await query()
				.select(($) => ({
					result,
				}))
			...
			const collected = Array.isArray(arr)
				? arr.sort()
				: logicListToArray(arr).sort();
			expect(results).toHaveLength(1);
	...
	describe("not", () => {
		...
		it("should work with variable bindings", async () => {
			const x = lvar("x");
			const goal = and(eq(x, 42), not(eq(x, 43)));
			const results = await query()
				.select(($) => ({
					x,
				}))
