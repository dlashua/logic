			...
			new SimpleObservable<Subst>((observer) => {
				...
				input$.subscribe({
					next: (s) => {
						...
						const distinctConstraints = suspends.filter((c) =>
							c.resumeFn.name.includes("distincto"),
						);
						const field = x.id?.split("_")[1];
						if (field && keyVals[field as keyof typeof keyVals]) {
							const usedValues = new Set();
							for (const constraint of distinctConstraints) {
								const relatedVars = constraint.watchedVars.filter(
									(id) => id.includes(`_${field}`) && id !== x.id,
								);
							...
							items = items.filter(
								(item) => !usedValues.has(JSON.stringify(item)),
							);
						}
