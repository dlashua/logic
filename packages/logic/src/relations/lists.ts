import { Goal, LogicList, Subst, Term } from "../core/types.js"
import {
  walk,
  unify,
  lvar,
...
import { getSuspendsFromSubst } from "../core/subst-suspends.js";

/**
 * A goal that succeeds if `x` is a member of the logic `list`.
 * Optimized for both arrays and logic lists.
 */
export function g2_membero(x: Term, list: Term): Goal {
...
// g1_membero
export function g1_membero(x: Term, list: Term): Goal {
  return enrichGroupInput("membero", [], [], (input$) =>
    new SimpleObservable<Subst>((observer) => {
      const subscriptions: any[] = [];
      let cancelled = false;
      ...
      input$.subscribe({
        next: (s) => {
          ...
          let items = Array.isArray(l) ? l : (isLogicList(l) ? logicListToArray(l) : []);

          // Filter items based on distincto constraints
          const suspends = getSuspendsFromSubst(s);
          const distinctConstraints = suspends.filter(c => c.resumeFn.name.includes("distincto"));
          const field = x.id?.split('_')[1];
          if (field && keyVals[field as keyof typeof keyVals]) {
            const usedValues = new Set();
            for (const constraint of distinctConstraints) {
              const relatedVars = constraint.watchedVars.filter(id => id.includes(`_${field}`) && id !== x.id);
              for (const varId of relatedVars) {
                const value = walk({ tag: "var", id: varId }, s);
                if (!isVar(value)) {
                  usedValues.add(JSON.stringify(value));
                }
              }
            }
            items = items.filter(item => !usedValues.has(JSON.stringify(item)));
          }
          ...
          if (Array.isArray(items)) {
            ...
            if (s1) observer.next(s1);
            active++;
            const sub = membero(x, l.tail)(SimpleObservable.of(s)).subscribe({
              next: observer.next,
              error: observer.error,
              complete: () => {
                active--;
    ...
    input$.subscribe({
      next: (s) => {
        ...
        if (isCons(xsVal)) {
          ...
          if (s1) {
            appendo(tail, ys, rest)(SimpleObservable.of(s1)).subscribe({
              next: observer.next,
              error: observer.error,
              complete: observer.complete
            });
          ...
          for (const head of arr) {
            ...
            and(
              ...
              next: (s1) => {
                ...
                if (isCons(ysVal2)) {
                  eq(ysVal2.tail, walk(lvar(), s1))(SimpleObservable.of(s1)).subscribe({
                    next: observer.next,
                    error: observer.error
                  });
                }
