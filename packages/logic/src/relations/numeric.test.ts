...
import { lvar, resetVarCounter } from "../core/kernel.js";
import { SimpleObservable } from "../core/observable.js";
import { query } from "../core/query.js";
import { SUSPENDED_CONSTRAINTS } from "../core/subst-suspends.js";
import {
	dividebyo,
	gteo,
	gto,
	...
	describe("pluso (addition constraint)", () => {
		...
		it("should work with constraint chains", async () => {
			const results = await query()
				...
				.where(($) => and(pluso($.x, $.y, $.z), eq($.x, 4), eq($.y, 6)))
				.toArray();
	...
	describe("minuso (addition constraint)", () => {
		...
		it("should work with constraint chains", async () => {
			const results = await query()
				...
				.where(($) => and(minuso($.x, $.y, $.z), eq($.x, 6), eq($.y, 4)))
				.toArray();
