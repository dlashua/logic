...
// import { getCousinsOf } from "../test/direct-sql.js";
...
export class FamilytreeRelations {
  ...
  grandparent_kid = (gp: Term<string>, k: Term<string>) =>
    this.ancestorOf(2)(k, gp);
  greatgrandparent_kid = (ggp: Term<string>, k: Term<string>) =>
    this.ancestorOf(3)(k, ggp);
  ...
  siblingOf = (v: Term<string>, s: Term<string>) => {
    ...
    return uniqueo(
      s,
      and(this.anyParentOf(v, in_s), this.anyKidOf(in_s, s), not(eq(v, s))),
    );
  };
  ...
  get uncleOf() {
    return this.uncleOfLevel(1);
  }
  get greatuncleOf() {
    return this.uncleOfLevel(2);
  }
  ...
  cousinOf(a: Term<string>, b: Term<string>, degree = 1, removal = 0): Goal {
    ...
    return uniqueo(b, and(...aUpGoals, ...bDownGoals, ...exclusions));
  }
  ...
  firstcousinsAgg = (v: Term<string>, s: Term<string[]>) =>
    this.cousinsAgg(v, s, 1);
  secondcousinsAgg = (v: Term<string>, s: Term<string[]>) =>
    this.cousinsAgg(v, s, 2);
  thirdcousinsAgg = (v: Term<string>, s: Term<string[]>) =>
    this.cousinsAgg(v, s, 3);
