...
import {
  ...
  makeSuspendHandler,
  suspendable,
} from "../../core/suspend-helper.ts";
import type { Goal, Subst, Term } from "../../core/types.ts";
import { neqo, not, thruCount } from "../../relations/control.ts";
...
export function make(piles, keyVals, keyBy, numArrays) {
  ...
  function unsetLater($, whereObj, updateObj) {
    function evaluator(values: unknown[], subst: Subst) {
      ...
      for (const updateKey of Object.keys(updateObj)) {
        function makeResumeFn(primaryValue, updateKey, updateVal) {
          return (subst) => {
            const primaryOtherValue = walk(
              $[`${keyBy}_${primaryValue}_${updateKey}`],
              subst,
            );
    ...
    return and(
      ...keyVals[keyBy].map((primaryValue) => {
        const watchVars = fields.map((field) =>
          field === keyBy
            ? primaryValue
            : $[`${keyBy}_${primaryValue}_${field}`],
        );
  ...
  function unlink($, objA, objB) {
    ...
    return and(unsetLater($, objA, objB), unsetLater($, objB, objA));
  }
  ...
  function rowConsistencyValidator($, rowVars) {
    const handler = (values, subst) => {
      ...
      const suspendSubst = addSuspendToSubst(
        resultSubst,
        handleSuspend,
        rowVars,
      );
  ...
  function distinctValidateAll($) {
    ...
    for (const field of fields) {
      ...
      constraints.push(
        manualDistinctnessValidator(row_collection, keyVals[field]),
      );
    }
      ...
      (values, subst) => {
        ...
        if (groundedValues.length > 0) {
          const uniqueValues = new Set(
            groundedValues.map((v) => JSON.stringify(v)),
          );
          ...
          if (
            field &&
            keyVals[field as keyof typeof keyVals] &&
            vars[0]?.id?.startsWith("q_p")
          ) {
            ...
            if (domain.length - usedValues.size < remainingVars) {
              console.log(
                `Distincto failed for ${field}: only ${domain.length - usedValues.size} values left for ${remainingVars} variables`,
              );
              return null;
            }
          }
