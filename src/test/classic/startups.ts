...
const keyVals = {
	name: ["atriana_com", "gofro_com", "protecha_com", "zetafish_com"],
	investment: [1000000, 2000000, 3000000, 4000000],
	founders: ["addie_abrams", "betty_becker", "fred_frost", "pat_padilla"],
};

const { constrainArrays, enforceConsistency, link, unlink, smartMemberoAll } =
	make(startups, keyVals, "name", 4);
...
const results = await query()
	...
	.where(($) => [
		...
		pluso(
			$.founders_betty_becker_investment,
			1000000,
			$.name_atriana_com_investment,
		),
		link($, {
			founders: "betty_becker",
			investment: $.founders_betty_becker_investment,
		}),
		...
		pluso(
			$.founders_addie_abrams_investment,
			2000000,
			$.name_gofro_com_investment,
		),
		link($, {
			founders: "addie_abrams",
			investment: $.founders_addie_abrams_investment,
		}),
