...
const pipeAsync = (...fns: ((val: any) => Promise<any> | any)[]) => (initialValue: any): Promise<any> => 
  fns.reduce((acc, fn) => acc.then(fn), Promise.resolve(initialValue));

const pluck = (key: string) => (rows: Record<string, any>[]) => rows.map(row => row[key]);

const flatMapObjectValues = <T>(rows: Record<string, T>[]): T[] =>
  rows.flatMap(row => Object.values(row));

const without = <T>(itemsToRemove: T[]) => (source: T[]): T[] =>
...
const flip = <A, B, C>(fn: (a: A) => (b: B) => C) => (b: B) => (a: A) => fn(a)(b);

const spread = <T, R>(fn: (...args: T[]) => R) => (args: T[]): R => fn(...args);
...
const query = (table: string) => (selectColumn: string | Record<string, string>) => (whereColumn: string) => (values: string[]) =>
...
const relationshipABQuery = (p: string[]) => getSharedDB()('relationship')
  .select('a', 'b')
  .whereIn('a', p)
  .orWhereIn('b', p);
...
const applyAndRemove = <T>(transform: (x: T[]) => Promise<T[]>) => async (items: T[]) =>
...
const getBioParents = pipeAsync(
  queryFamily('parent')('kid'),
  pluck('parent')
);

const getBioChildren = pipeAsync(
  queryFamily('kid')('parent'),
  pluck('kid')
);

const getParentsOf = pipeAsync(
  getBioParents,
  withPartners
);

const getChildrenOf = pipeAsync(
  withPartners,
  getBioChildren,
  distinct
);
...
const repeat = (count: number) => (fn: (val: any) => Promise<any> | any) => 
  pipeAsync(...Array.from({
    length: Math.abs(count) 
  }, () => fn));
  
const getAncestorsAtGeneration = (generations: number) => repeat(generations)(getParentsOf);
const getDescendantsAtGeneration = (generations: number) => repeat(generations)(getChildrenOf);
...
const getAllDescendantsAndSelf = async (people: string[]): Promise<string[]> => {
  if (people.length === 0) {
    return [];
  }
...
const findCousinPool = (degree: number, removal: number) => pipeAsync(
  fanout(
...
export const getCousinsOf = (personName: string, degree = 1, removal = 0): Promise<string[]> => 
  findCousinPool(degree, removal)([personName]);
