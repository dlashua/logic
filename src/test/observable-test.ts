...
import { Subscription } from "../core/types.ts";
...
await dumpObs("map 1", BaseObservable.map((x) => x+5))
await dumpObs("map array", BaseObservable.map((x) => [x+5, x+105]))

await dumpObs("filter", BaseObservable.filter((x) => x % 2 === 0))

await dumpObs("take 3", BaseObservable.take(3));

const shared = BaseObservable.share();
await dumpObs("share 1 take", shared.take(3));
await dumpObs("share 2 filter", shared.filter((x) => x % 2 === 0))
await dumpObs("share 3 take", shared.take(3));

await dumpObs("flatmap 1", BaseObservable.flatMap((x) => SimpleObservable.of(x+5)))
await dumpObs("flatmap 2", BaseObservable.flatMap((x) => SimpleObservable.of(x+5, x+105)))
await dumpObs("flatmap array", BaseObservable.flatMap((x) => SimpleObservable.of([x+5, x+105])))
...
await dumpObs("async filter", BaseAsyncObservable.filter((x) => x % 2 === 0))
